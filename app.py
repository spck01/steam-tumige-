...
- **PC**では、Xの共有先がOSに登録されていないことが多いため、生成画像をブラウザの機能でクリップボードに自動コピーしたうえで、Xの投稿画面を新しいタブで開きます（投稿欄への貼り付けはユーザー自身の操作が必要です）。
...
- 本アプリが利用しているフレームワーク（Streamlit）自体の匿名利用統計送信も無効化しています。
    ...
    with st.expander("📝 生成画像の著作権について"):
        st.markdown("""
生成される画像には各ゲームのアイコン（各ゲーム会社が著作権を持つ画像）を使用しています。この点は認識した上で、以下を踏まえて提供しています。
...
同種のライブラリ可視化ツールは既に広く使われており、この用途での権利行使の実例は確認されていません。より詳しい考え方は[README](https://github.com/spck01/steam-tumige-#生成画像の著作権に関する考え方)にも記載しています。
        """)