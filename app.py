...
- ログイン後にSteamから返される認証情報は、Steam側に問い合わせて正当性を検証したうえで信頼します（第三者によるなりすましを防止するためです）。
...
- 合成された画像も同様にサーバー上には保存されません（お使いの端末上でのみダウンロード・共有されます）。

#### X（旧Twitter）への共有機能について
- **スマートフォン**では、お使いの端末のOS標準の共有機能（Web Share API）を呼び出し、画像とテキストを共有シートへ渡します。
- **PC**では、Xの共有先がOSに登録されていないことが多いため、合成画像をブラウザの機能でクリップボードに自動コピーしたうえで、Xの投稿画面を新しいタブで開きます（投稿欄への貼り付けはユーザー自身の操作が必要です）。
- いずれの場合も、画像はお使いの端末のブラウザ内で処理され、本アプリのサーバーを経由することはありません。
- 投稿文には本アプリへのリンクを含めていますが、内容は投稿前にX側の画面で確認・編集できます。
- 実際に投稿するかどうかは、X側の画面でユーザー自身が操作するまで確定しません（自動投稿は行いません）。
...
- 本アプリが利用しているフレームワーク（Streamlit）自体の匿名利用統計送信も無効化しています。
        """)

    with st.expander("📝 合成画像の著作権について"):
        st.markdown("""
合成される画像には各ゲームのアイコン（各ゲーム会社が著作権を持つ画像）を使用しています。この点は認識した上で、以下を踏まえて提供しています。

- アイコンは**Steam公式のCDN**から取得しており、Valveがライブラリ表示用に公開している画像をそのまま利用しています
- 表示サイズは**64×64pxの小さなサムネイル**で、ゲーム本体やアートワークの価値を代替するものではありません
- あくまで**自分自身の所持ライブラリを示す個人利用**が目的で、Steamクライアントのライブラリ表示を画像化しているに過ぎません
- **非商用**で提供しており、収益化は行っていません

同種のライブラリ可視化ツールは既に広く使われており、この用途での権利行使の実例は確認されていません。より詳しい考え方は[README](https://github.com/spck01/steam-tumige-#合成画像の著作権に関する考え方)にも記載しています。
        """)