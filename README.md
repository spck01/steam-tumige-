...
- OGP（Xなどでのリンクプレビュー）は、Streamlitの標準機能では設定できないため、`app.py`が起動時にインストール済みの`index.html`へ非公式にメタタグを書き込む方式で対応している（[app.py](app.py)の`_patch_ogp_meta_tags()`）。コンテナが再起動・スリープから復帰した直後は、誰か1人がアプリにアクセスしてスクリプトが実行されるまでメタタグが反映されない点に注意。またStreamlitの内部実装変更で動作しなくなる可能性がある
