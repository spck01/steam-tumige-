...
- [生成画像の著作権に関する考え方](#生成画像の著作権に関する考え方)
- [セキュリティ対策](#セキュリティ対策)
...
| 積みゲー判定基準 | 累計プレイ時間が **3時間（180分）以下** のゲーム（`TSUMI_THRESHOLD_MINUTES`で定義） |
...
生成画像は各ゲームのアイコン画像（各ゲーム会社が著作権を持つ著作物）をそのまま並べたものであり、厳密には著作物の複製にあたる。この点は認識した上で、以下の理由からリスクは低いと判断し、現状のまま運用している（法的な確約ではなく、一般的な実務慣行に基づく判断）。
...
将来的に利用規模が大きくなった場合や、より慎重を期したい場合は、生成画像またはツイート文に「各ゲームのアイコン・商標は各権利者に帰属します」といった注記を追加することを検討する。
